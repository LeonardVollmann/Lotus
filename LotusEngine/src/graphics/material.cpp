...
namespace lotus { namespace graphics {
	...
	Material::Material(Texture *diffuse, maths::vec4f color, float specularIntensity, float specularPower, Texture *normalMap, Texture *dispMap, float dispMapScale, float dispMapOffset)
	{
		set("diffuse", diffuse);
	...
	Material::~Material()
	{
		for (auto it = m_values.begin(); it != m_values.end(); it++)
		{
			...
			else if (type == typeid(maths::vec2f*)) delete (maths::vec2f*) it->second;
			else if (type == typeid(maths::vec3f*)) delete (maths::vec3f*) it->second;
			else if (type == typeid(maths::vec4f*)) delete (maths::vec4f*) it->second;
