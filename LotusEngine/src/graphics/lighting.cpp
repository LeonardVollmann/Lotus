...
namespace lotus { namespace graphics {
	...
	Attenuation::Attenuation(const maths::vec3 &attenuation) :
	...
	BaseLight::BaseLight(const maths::vec3 &color, float intensity) :
		m_color(color),
	...
	DirectionalLight::DirectionalLight() :
		BaseLight(),
		m_direction(maths::vec3(1.0f, 1.0f, 1.0f).normalize()) {}

	DirectionalLight::DirectionalLight(const maths::vec3 &color, float intensity, const maths::vec3 &direction) :
		BaseLight(color, intensity),
		m_direction(direction) {}

	DirectionalLight::DirectionalLight(const maths::vec3 &direction, const BaseLight &base) :
		BaseLight(base),
		m_direction(direction) {}
	...
	PointLight::PointLight(const maths::vec3 &color, float intensity, const Attenuation &atten, const maths::vec3 &pos) :
		BaseLight(color, intensity),
		m_atten(atten),
		m_pos(pos),
		m_range(calcRange(*this)) {}

	PointLight::PointLight(const Attenuation &atten, const maths::vec3 &pos, const BaseLight &base) :
		BaseLight(base),
		m_atten(atten),
		m_pos(pos),
		m_range(calcRange(*this)) {}
	...
	SpotLight::SpotLight(const maths::vec3 &color, float intensity, const Attenuation &atten, const maths::vec3 &pos, const maths::vec3 &direction, float cutoff) :
		PointLight(color, intensity, atten, pos),
		m_direction(direction),
		m_cutoff(cutoff) {}

	SpotLight::SpotLight(const maths::vec3 &direction, float cutoff, const PointLight &pointLight) :
		PointLight(pointLight),
		m_direction(direction),
		m_cutoff(cutoff) {}
