...
public:
	...
	Material(Texture *diffuse,
	         maths::vec4f color      = maths::vec4f(),
	         float specularIntensity = 2.0f,
	         float specularPower     = 8.0f,
	         Texture *normalMap      = new Texture("default_normal.png"),
	...
	inline const maths::vec2f *getVector2f(const std::string &name) const
	{
		return (const maths::vec2f *)get(name);
	}
	inline const maths::vec3f *getVector3f(const std::string &name) const
	{
		return (const maths::vec3f *)get(name);
	}
	inline const maths::vec4f *getVector4f(const std::string &name) const
	{
		return (maths::vec4f *)get(name);
	}
	inline const Texture *getTexture(const std::string &name) const
	{
		return (const Texture *)get(name);
	}
};
