...
namespace lotus { namespace maths {
	...
	inline Vector<T, V_N> mul(const Matrix<T, M_N> &l, const Vector<T, V_N> &r)
	{
	...
	template <typename T, unsigned int M_N, unsigned int V_N>
	inline Vector<T, V_N> operator*(const Matrix<T, M_N> &l, const Vector<T, V_N> &r) { return mul(l, r); }
