...
//			if (Input::getCursorPosDelta() != maths::vec2(0.0f)) std::cout << Input::getCursorPosDelta() << std::endl;
		...
		else
		{
			...
			{
				m_entity->getTransform().rotate(maths::getRight(m_entity->getTransform().getRot()), m_turnSpeed * delta);
			}
			if (Input::getKey(GLFW_KEY_DOWN))
			{
				m_entity->getTransform().rotate(maths::getLeft(m_entity->getTransform().getRot()), m_turnSpeed * delta);
			}
