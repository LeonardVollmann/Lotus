...
template <typename T, unsigned int N>
inline mat<T, N> operator*(const mat<T, N> &l, const mat<T, N> &r)
{
	return mul(l, r);
}

template <typename T, unsigned int M_N, unsigned int V_N>
inline vec<T, V_N> operator*(const mat<T, M_N> &l, const vec<T, V_N> &r)
{
	return mul(l, r);
}

template <typename T, unsigned int N>
inline mat<T, N> operator*(const mat<T, N> &l, T r)
{
	return mul(l, r);
}
